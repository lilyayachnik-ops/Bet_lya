...
Установим `Docker` на свою систему, как указано в официальной документации. Запустим запись вывода терминала в файл. Запустим контейнер на порту `28080` из официального образа `nginx`: `docker run -d -p 127.0.0.1:28080:80 --name rbm-dkr-01 nginx:stable`. Выведим в консоли список запущенных контейнеров командой: `docker ps`. При помощи утилиты `curl` запроси адрес `http://127.0.0.1:28080` - должна появиться приветственная страница `nginx`. Если не появляется, то запуск контейнера неуспешен по какой-то причине.

Покажем, что у нас работает `Docker`, используя команду `docker run hello-world`:

<img width="1118" height="632" alt="image" src="https://github.com/user-attachments/assets/7c2347da-e753-42e5-9224-9912b1a78cde" />
...
Остановим ранее запущенный контейнер командой: `docker stop rbm-dkr-01`. Выполним команду `docker ps`, тем самым проверим произошедшие изменения в списке запущенных контейнеров — список контейнеров должен быть пуст. При помощи утилиты `curl` запросим адрес `http://127.0.0.1:28080` — сейчас должна быть ошибка, поскольку мы уже удалили контейнер и ничего не слушается на этом порту:
...
При помощи команды `docker ps -a` выведим список всех контейнеров в системе — в списке будет нами остановленный контейнер. Остановим запись в файл и загрузите полученные логи в репозиторий на `gitlab`:
