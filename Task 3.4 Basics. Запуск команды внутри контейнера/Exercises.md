...
Скачай новый конфигурационный файл `nginx`. Измени проброшенный конфигурационный файл, размещенный на хостовой системе, на новый. Проведи эксперименты с различными способами изменения: через команду `cp`, через `vim`, `nano`. Выполни `reload nginx` без остановки контейнера при помощи команды `exec`. Проверь работу, обратившись к `127.0.0.1:8890`, — в ответ должно возвращать строку `Welcome to the training program Innowise: Docker! Again!`
Выполни команду `docker exec -ti inno-dkr-03 md5sum /etc/nginx/nginx.conf` (вывод в консоли сохраняем). Сравни результаты изменения файла `nginx.conf` разными командами. Найди информацию, почему в некоторых случаях изменение не отображалось в контейнере. Выведи список запущенных контейнеров — контейнер должен быть запущен.
