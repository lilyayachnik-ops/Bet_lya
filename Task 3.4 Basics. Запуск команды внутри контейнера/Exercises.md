...
Скачаем конфигурационный файл nginx. Запустим контейнер со следующими параметрами: должен работать в фоне, слушает на хосте `127.0.0.1:8890`,
имеет имя `inno-dkr-03`, должен пробрасывать скачанный нами конфигурационный файл внутрь контейнера как основной конфигурационный файл, образ — `nginx:stable`. Проверим работу, обратившись к `127.0.0.1:8890`, — в ответ должно возвращать строку `Welcome to the training program Innowise: Docker`. Выведим список запущенных контейнеров — контейнер должен быть запущен. Выполним команду `docker exec -ti inno-dkr-03 md5sum /etc/nginx/nginx.conf`. Скачаем новый конфигурационный файл `nginx`. Изменим проброшенный конфигурационный файл через `cp`, размещенный на хостовой системе, на новый. Выполним `reload nginx` без остановки контейнера при помощи команды `exec`. Проверим работу, обратившись к `127.0.0.1:8890`, — в ответ должно возвращать строку `Welcome to the training program Innowise: Docker! Again!`. Выполним команду `docker exec -ti inno-dkr-03 md5sum /etc/nginx/nginx.conf` (вывод в консоли сохраняем).:
...
Проведём эксперименты с различными способами изменения: через `vim`, `nano`. Сравним результаты изменения файла `nginx.conf` разными командами. Найдём информацию, почему в некоторых случаях изменение не отображалось в контейнере. Выведим список запущенных контейнеров — контейнер должен быть запущен.
