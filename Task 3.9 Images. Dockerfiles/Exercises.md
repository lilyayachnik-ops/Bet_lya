...
Скачай конфигурационный файл `nginx`. Опиши Dockerfile, в котором: как базовый образ используется `nginx:stable`, внутрь контейнера как основной конфигурационный файл копируется скачанный тобой `nginx.conf`. Собери этот образ с именем `nginx` и тегом `inno-dkr-08`. Выведи список образов на твоем хосте:
...
Запусти контейнер со следующими параметрами: образ — собранный нами образ, должно работать в фоне, слушает на хосте `127.0.0.1:8900`. Выведи список запущенных контейнеров — контейнер должен быть запущен. Проверь работу, обратившись к `127.0.0.1:8900`, — в ответ должно возвращать строку: `Welcome to the training program Innowise: Docker!`:
