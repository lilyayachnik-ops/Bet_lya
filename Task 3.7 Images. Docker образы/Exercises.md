...
Выполни загрузку образа `nginx:stable-alpine` на свой локальный хост. Добавь к загруженному образу новый тег `inno-dkr-07` — имя репозитория должно оставаться таким же, поменяться только тег. Выведи список образов на твоем хосте:
...
Запусти контейнер со следующими параметрами: контейнер должен работать в фоне, образ, который должен быть получен в результате переименования образа `nginx:stable-alpine`:
