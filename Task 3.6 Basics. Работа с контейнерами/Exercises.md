...
Запустим контейнер со следующими параметрами: должно работать в фоне, имеет имя `inno-dkr-05-run-X`, где `X` — набор из `10` случайных букв и/или цифр которая должна генерироваться в момент запуска контейнера (можно использовать команду `cat /dev/urandom | tr -cd 'a-f0-9' | head -c 10`),
образ — `nginx:stable`, команда для запуска обоих контейнеров должна быть одинаковой (выполнить одинаковую команду два раза подряд). Запустим контейнер со следующими параметрами: должно работать в фоне, имеет имя `inno-dkr-05-stop`, образ — `nginx:stable`:
...
Выполним команду `docker ps`, вывод перенаправим в файл `/home/user/ps.txt` (`docker ps | tee /home/user/ps.txt`). Остановим контейнер `inno-dkr-05-stop`. Выведим список всех контейнеров. Одной командой остановим все запущенные контейнеры. Выведим список всех контейнеров. Одной командой удалим все контейнеры, любой из разобранных методов подходит:
